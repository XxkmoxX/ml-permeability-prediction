...
![binary_colecchio](plots/binary_colecchio.png)                                              
  ...
  **c.** Calculation of connectivity indicators using the Connect3D code derived from percolation theory and integral geometry. Connectivity characterization has several approaches, and there is no unified view. This work uses some of the static connectivity indicators frequently used in the literature, derived from percolation theory and integral geometry. These indicators are combined to study phase transitions in percolation systems and topological characteristics of connectivity. In integral geometry, they are also linked to tools such as correlation functions and measures related to the properties of networks and stochastic processes (Pardo and Dowd, 2003).<br>
  ...
  - **VOLALE:** Relative volume of the largest cluster. It is the ratio of the number of cells comprising the largest cluster of high-permeability cells to the total number of high-permeability cells. It takes values ​​between 0 and 1.<br>
  - **GAMMA:** Ratio of the number of connected pairs of high hydraulic conductivity cells to all possible pairs of high hydraulic conductivity cells in the domain. Where ***n<sub>p<\sub>*** is the total number of high hydraulic conductivity cells in the model and ***n<sub>i<\sub>*** is the number of permeable cells belonging to the *i<sup>th<\sup> cluster*. This indicator takes values ​​between 0 and 1.<br>
  - **ZNCC:** Number of clusters connecting opposite sides of the medium in the direction of flow, i.e., those that percolate. In statistical terms, when ***p*** is close to the percolation threshold value ***p<sub>c<\sub>***, ZNCC will be equal to or greater than unity. This indicator takes integer values.<br>
  - **SPANNING:** Spanning cluster indicates whether percolation is present, that is, whether flow exists in the 3D cells. Individual components of the spanning cluster can be used: npx and npy for the 2D case.<br>
  - **PPHA (p):** Proportion of sites occupied by high hydraulic conductivity classes *k(**r**)*.<br>
  - **S:** Finite average number of connections in the system. It represents the number of points that are part of a typical cluster before the system percolates (i.e., before an infinite cluster exists). This is crucial for identifying the transition to percolation, as it tends to diverge right at the threshold.<br>
  - **X:** Infinite cluster density. This indicator describes the probability that a random point in the system belongs to an infinite cluster. It is zero when the system is below the percolation threshold and takes positive values ​​when an infinite cluster arises.<br>
  - **Plen:** Describes the probability that two points separated by a given distance in a system are connected. It is a decay function that captures how connectivity decreases as the distance between points increases.<br><br>
